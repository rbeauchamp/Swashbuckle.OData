...
namespace Swashbuckle.OData.ApiExplorer
{
    ...
    public class ApiExplorerFromMicrosoft : IApiExplorer
    {
        ...
        /// <summary>
        ///     Determines whether the action should be considered for <see cref="ApiExplorer.ApiDescriptions" /> generation.
        ///     Called when initializing the <see cref="ApiExplorer.ApiDescriptions" />.
        /// </summary>
        ...
        /// <param name="route">The route.</param>
        /// <returns>
        ///     <c>true</c> if the action should be considered for <see cref="ApiExplorer.ApiDescriptions" /> generation,
        ///     <c>false</c> otherwise.
        /// </returns>
        public virtual bool ShouldExploreAction(string actionVariableValue, HttpActionDescriptor actionDescriptor, IHttpRoute route)
        {
            ...
            return (setting == null || !setting.IgnoreApi) && MatchRegexConstraint(route, RouteValueKeys.Action, actionVariableValue);
        }
        ...
        public virtual Collection<HttpMethod> GetHttpMethodsSupportedByAction(IHttpRoute route, HttpActionDescriptor actionDescriptor)
        {
            ...
            var httpMethodConstraint = route.Constraints.Values.FirstOrDefault(c => typeof (HttpMethodConstraint).IsAssignableFrom(c.GetType())) as HttpMethodConstraint;
        ...
        private static HttpControllerDescriptor GetDirectRouteController(CandidateAction[] directRouteCandidates)
        {
            if (directRouteCandidates != null)
            {
                ...
                for (var i = 1; i < directRouteCandidates.Length; i++)
                {
        ...
        private Collection<ApiDescription> InitializeApiDescriptions()
        {
            ...
            if (controllerMappings != null)
            {
                ...
                foreach (var route in FlattenRoutes(_config.Routes))
                {
                    ...
                    var directRouteController = GetDirectRouteController(directRouteCandidates);
                    var descriptionsFromRoute = directRouteController != null && directRouteCandidates != null ? ExploreDirectRoute(directRouteController, directRouteCandidates, route) : ExploreRouteControllers(controllerMappings, route);
        ...
        private Collection<ApiDescription> ExploreRouteControllers(IDictionary<string, HttpControllerDescriptor> controllerMappings, IHttpRoute route)
        {
            ...
            if (_controllerVariableRegex.IsMatch(routeTemplate))
            {
                ...
                foreach (var controllerMapping in controllerMappings)
                {
                    ...
                    var controllerDescriptor = controllerMapping.Value;
                    if (ShouldExploreController(controllerVariableValue, controllerDescriptor, route))
                    {
                        // expand {controller} variable
                        var expandedRouteTemplate = _controllerVariableRegex.Replace(routeTemplate, controllerVariableValue);
                ...
                if (actionMappings != null)
                {
                    if (_actionVariableRegex.IsMatch(localPath))
                    {
                        ...
                        foreach (var actionMapping in actionMappings)
                        {
                            ...
                            var expandedLocalPath = _actionVariableRegex.Replace(localPath, actionVariableValue);
        ...
        private void PopulateActionDescriptions(HttpActionDescriptor actionDescriptor, IHttpRoute route, string localPath, Collection<ApiDescription> apiDescriptions)
        {
            ...
            var supportedRequestBodyFormatters = bodyParameter != null ? actionDescriptor.Configuration.Formatters.Where(f => f.CanReadType(bodyParameter.ParameterDescriptor.ParameterType)) : Enumerable.Empty<MediaTypeFormatter>();
            ...
            var supportedResponseFormatters = returnType != null && returnType != typeof (void) ? actionDescriptor.Configuration.Formatters.Where(f => f.CanWriteType(returnType)) : Enumerable.Empty<MediaTypeFormatter>();
        ...
        {
            foreach (var formatter in mediaTypeFormatters)
            {
        ...
        private static bool ShouldEmitPrefixes(ICollection<ApiParameterDescription> parameterDescriptions)
        {
            ...
            return parameterDescriptions.Count(parameter => parameter.Source == ApiParameterSource.FromUri && parameter.ParameterDescriptor != null && !TypeHelper.CanConvertFromString(parameter.ParameterDescriptor.ParameterType) && parameter.CanConvertPropertiesFromString()) > 1;
        }
            ...
            foreach (var parameterDescription in parameterDescriptions)
            {
                if (parameterDescription.Source == ApiParameterSource.FromUri)
                {
                    ...
                    else if (IsBindableCollection(parameterDescription.ParameterDescriptor.ParameterType))
                    {
                        ...
                        var innerTypeProperties = ApiParameterDescriptionExtensions.GetBindableProperties(innerType).ToArray();
                    ...
                    else if (parameterDescription.CanConvertPropertiesFromString())
                    {
                        ...
                        AddPlaceholderForProperties(parameterValuesForRoute, parameterDescription.ParameterDescriptor.GetBindableProperties(), prefix);
                    }
                }
            }
        ...
        private static Type GetCollectionElementType(Type collectionType)
        {
            ...
            if (elementType == null)
            {
                elementType = CollectionModelBinderUtil.GetGenericBinderTypeArgs(typeof (ICollection<>), collectionType).First();
            }
            return elementType;
        }
        ...
        private static void AddUndeclaredRouteParameters(HttpParsedRoute parsedRoute, IDictionary<string, object> routeDefaults, IList<ApiParameterDescription> parameterDescriptions)
        {
            ...
            {
                var content = path as PathContentSegment;
                if (content != null)
                {
                    foreach (var subSegment in content.Subsegments)
                    {
                        var parameter = subSegment as PathParameterSubsegment;
                        if (parameter != null)
                        {
                            object parameterValue;
                            var parameterName = parameter.ParameterName;
                            if (!parameterDescriptions.Any(p => string.Equals(p.Name, parameterName, StringComparison.OrdinalIgnoreCase)) && (!routeDefaults.TryGetValue(parameterName, out parameterValue) || parameterValue != RouteParameter.Optional))
                            {
        ...
        {
            var documentationProvider = DocumentationProvider ?? actionDescriptor.Configuration.Services.GetDocumentationProvider();
        ...
        {
            var documentationProvider = DocumentationProvider ?? parameterDescriptor.Configuration.Services.GetDocumentationProvider();
            if (documentationProvider != null)
            {
        ...
        private static HttpActionBinding GetActionBinding(HttpActionDescriptor actionDescriptor)
        {
            ...
            var actionBinding = actionValueBinder != null ? actionValueBinder.GetBinding(actionDescriptor) : null;
