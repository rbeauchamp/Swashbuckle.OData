    ...
    internal class RouteCollectionRoute : IHttpRoute, IReadOnlyCollection<IHttpRoute>
    {
        ...
        private IReadOnlyCollection<IHttpRoute> SubRoutes
        {
            get
            {
                ...
                if (_subRoutes == null)
                {
                    var msg = Error.Format("The object has not yet been initialized. Ensure that HttpConfiguration.EnsureInitialized() is called in the application's startup code after all other initialization code.");
        ...
        {
            var matches = new List<IHttpRouteData>();
            foreach (var route in SubRoutes)
            {
                var match = route.GetRouteData(virtualPathRoot, request);
                if (match != null)
                {
                    matches.Add(match);
                }
