...
namespace Swashbuckle.OData.ApiExplorer
{
    internal sealed class HttpParsedRoute
    {
        ...
        public BoundRouteTemplate Bind(IDictionary<string, object> currentValues, IDictionary<string, object> values, HttpRouteValueDictionary defaultValues, HttpRouteValueDictionary constraints)
        {
            ...
            var acceptedValues = new HttpRouteValueDictionary();
            ...
            var hasAllRequiredValues = ForEachParameter(PathSegments, delegate(PathParameterSubsegment parameterSubsegment)
            {
                object defaultValue;
                if (IsParameterRequired(parameterSubsegment, defaultValues, out defaultValue))
                {
            ...
            var otherDefaultValues = new HttpRouteValueDictionary(defaultValues);
            ForEachParameter(PathSegments, delegate(PathParameterSubsegment parameterSubsegment)
            {
            ...
            var blockAllUriAppends = false;

            for (var i = 0; i < PathSegments.Count; i++)
            {
                var pathSegment = PathSegments[i]; // parsedRouteUriPart
                ...
                {
                    var contentPathSegment = pathSegment as PathContentSegment;
                    if (contentPathSegment != null)
                    {
                        ...
                        for (var j = 0; j < contentPathSegment.Subsegments.Count; j++)
                        {
                            var subsegment = contentPathSegment.Subsegments[j];
        ...
        {
            var requestPathSegments = context.PathSegments;
            ...
            var matchedValues = new HttpRouteValueDictionary();
        ...
        private static void MatchCatchAll(PathContentSegment contentPathSegment, IEnumerable<string> remainingRequestSegments, HttpRouteValueDictionary defaultValues, HttpRouteValueDictionary matchedValues)
        {
            ...
            var catchAllSegment = contentPathSegment.Subsegments[0] as PathParameterSubsegment;
        ...
        private static bool MatchContentPathSegment(PathContentSegment routeSegment, string requestPathSegment, HttpRouteValueDictionary defaultValues, HttpRouteValueDictionary matchedValues)
        {
            if (string.IsNullOrEmpty(requestPathSegment))
            {
                ...
                var parameterSubsegment = routeSegment.Subsegments[0] as PathParameterSubsegment;
                if (parameterSubsegment == null)
                {
                    return false;
                }
            ...
            while (indexOfLastSegmentUsed >= 0)
            {
                ...
                var parameterSubsegment = routeSegment.Subsegments[indexOfLastSegmentUsed] as PathParameterSubsegment;
                if (parameterSubsegment != null)
                {
                ...
                {
                    var literalSubsegment = routeSegment.Subsegments[indexOfLastSegmentUsed] as PathLiteralSubsegment;
                    if (literalSubsegment != null)
                    {
        ...
        {
            var parameterSubsegment = pathSubsegment as PathParameterSubsegment;
            if (parameterSubsegment == null)
            {
                // Handle a single literal segment
                var literalSubsegment = pathSubsegment as PathLiteralSubsegment;
